...
class AnimationEditor extends React.Component{
	...
	this.currentStepAV.addListener(progress => {this.currentStep = progress.value;
        });
    }
	...
	console.log("Animation : positionPercentToPixel, animation width/height: " + this.state.width + "/" + (this.state.height * this.hRatio));
	...
	console.log("currentStep: " + this.currentStep + " ceil: " + Math.ceil(this.currentStep));
        ...
        var currentPosition = this.state.drill.getPositionsAtStep(element.props.eId, Math.ceil(this.currentStep))[0]; 
	var xDeltaPercent = xDelta / (this.state.width * this.wRatio);
	var yDeltaPercent = yDelta / (this.state.height * this.hRatio);

	console.log("moved element to position: " + (currentPosition[0] + xDeltaPercent) + "/" + (currentPosition[1]+ yDeltaPercent));  
